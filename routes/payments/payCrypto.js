...
router.post('/', (req, res, next) => {
    ...
    let orderDoc = {
        ...
        orderPaymentGateway: 'payCrypto',               // 페이방식
...
router.get('/check', (req, res, next) => {
  ...
  if(req.query.result == "0") {
    ...
    var orderNumber = req.query.ordernum;

    let db = req.app.db;

    db.orders.find({orderPaymentId: orderNumber}).toArray((err, result) => {
      ...
      if(result.length == 0) {
        ...
        db.orders.update({orderPaymentId: orderNumber}, {$set: {orderStatus: "1"}});

        console.log("order status updated");
        res.redirect('/paycrypto/completed')
      }
    });
  }
});
...
router.get('/completed', (req, res, next) => {
  ...
  res.render(`${config.themeViews}payCrypto_complete`, {
